...
void CameraPipeline::handle_pad_added(GstPad* new_pad) {
    ...
    // 2. caps 전체를 문자열로 출력한다.
    //    25강의 핵심은 실제 RTSP 서버가 어떤 RTP 정보를 주는지 확인하는 것이다.
    ...
    // 4. video RTP인지 확인한다.
    //    오디오 pad가 들어올 수 있으므로 media=video만 처리한다.
    ...
    // 5. encoding-name 확인.
    //    앞으로 H264/H265/MJPEG/MPEG 계열을 구분할 기준이다.
    ...
    // 6. 25강 시점에서는 아직 H264만 실제 연결한다.
    //    H265/MJPEG/MPEG 계열은 여기서 감지만 하고 unsupported로 남긴다.
    if (encoding_name == nullptr ||
        g_ascii_strcasecmp(encoding_name, "H264") != 0) {
        std::cout << "[" << camera_id_ << "] Unsupported RTP video encoding-name for now: "
                  << (encoding_name ? encoding_name : "none")
                  << std::endl;
    ...
    // 7. rtph264depay의 sink pad를 가져온다.
    ...
    // 8. 이미 연결되어 있으면 같은 pad를 중복 연결하지 않는다.
    ...
    // 9. rtspsrc의 동적 pad를 rtph264depay에 연결한다.
