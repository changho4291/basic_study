#include <gst/gst.h>

#include <iostream>
#include <string>

struct AppData {
    GstElement* depay;
};

static void on_pad_added(GstElement* src, GstPad* new_pad, gpointer user_data) {
    (void)src;

    AppData* data = static_cast<AppData*>(user_data);

    // 1. rtph264depay의 sinp pad 가져오기
    GstPad* sink_pad = gst_element_get_static_pad(data->depay, "sink");

    if (sink_pad == nullptr) {
        std::cerr << "Failed to get depay sink pad" << std::endl;
        return;
    }

    // 2. 이미 연결되어 있으면 무시
    if (gst_pad_is_linked(sink_pad)) {
        std::cout << "Depay sink pad is already linked" << std::endl;
        gst_object_unref(sink_pad);
        return;
    }

    // 3. 새 pad의 caps 확인
    GstCaps* new_pad_caps = gst_pad_get_current_caps(new_pad);

    if (new_pad_caps == nullptr) {
        new_pad_caps = gst_pad_query_caps(new_pad, nullptr);
    }

    if (new_pad_caps == nullptr || gst_caps_get_size(new_pad_caps) == 0) {
        std::cerr << "Failed to get new pad caps" << std::endl;

        if (new_pad_caps != nullptr) { gst_caps_unref(new_pad_caps); }

        gst_object_unref(sink_pad);
        return;
    }

    // 4. caps 출력
    gchar* caps_str = gst_caps_to_string(new_pad_caps);
    std::cout << "New pad caps: " << (caps_str ? caps_str : "unknown") << std::endl;

    if (caps_str != nullptr) { g_free(caps_str); }

    GstStructure* new_pad_struct = gst_caps_get_structure(new_pad_caps, 0);
    const gchar* new_pad_type = gst_structure_get_name(new_pad_struct);

    // 5. RTP pad인지 확인
    if (g_str_has_prefix(new_pad_type, "application/x-rtp") == FALSE) {
        std::cout << "It is not RTP. Ignore this pad." << std::endl;

        gst_caps_unref(new_pad_caps);
        gst_object_unref(sink_pad);
        return;
    }

    // 6. H264 RTP인지 확인
    const gchar* encoding_name = gst_structure_get_string(
        new_pad_struct,
        "encoding-name"
    );

    std::cout << "RTP encoding-name: "
              << (encoding_name ? encoding_name : "none")
              << std::endl;

    if (encoding_name == nullptr || g_ascii_strcasecmp(encoding_name, "H264") != 0) {
        std::cout << "It is not H264 RTP. Ignore this pad." << std::endl;

        gst_caps_unref(new_pad_caps);
        gst_object_unref(sink_pad);
        return;
    }

    // 7. rtspsrc의 동적 pad를 rtph264depay에 연결
    GstPadLinkReturn link_ret = gst_pad_link(new_pad, sink_pad);

    if (GST_PAD_LINK_FAILED(link_ret)) {
        std::cerr << "Failed to link rtspsrc dynamic pad" << std::endl;
    } else {
        std::cout << "RTSP dynamic pad linked successfully" << std::endl;
    }

    // 8. 참조 해제
    gst_caps_unref(new_pad_caps);
    gst_object_unref(sink_pad);
}

int main(int argc, char* argv[]) {
    // 1. GStreamer 초기화
    gst_init(&argc, &argv);

    // 2. RTSP URI / 저장 파일 패턴 설정
    std::string rtsp_uri = "rtsp://127.0.0.1:8554/test";
    std::string output_pattern = "record_%03d.mp4";

    if (argc >= 2) { rtsp_uri = argv[1]; }
    if (argc >= 3) { output_pattern = argv[2]; }

    std::cout << "RTSP URI: " << rtsp_uri << std::endl;
    std::cout << "Output pattern: " << output_pattern << std::endl;

    // 3. 파이프라인 생성
    GstElement* pipeline = gst_pipeline_new("tee-queue-pipeline");

    // 4. Element 생성
    GstElement* source = gst_element_factory_make("rtspsrc", "source");
    GstElement* depay = gst_element_factory_make("rtph264depay", "depay");
    GstElement* parser = gst_element_factory_make("h264parse", "parser");
    GstElement* tee = gst_element_factory_make("tee", "tee");

    GstElement* record_queue = gst_element_factory_make("queue", "record-queue");
    GstElement* split_sink = gst_element_factory_make("splitmuxsink", "split-sink");

    GstElement* fake_queue = gst_element_factory_make("queue", "fake-queue");
    GstElement* fake_sink = gst_element_factory_make("fakesink", "fake-sink");

    // 5. Element 생성 확인
    if (pipeline == nullptr || source == nullptr || depay == nullptr ||
        parser == nullptr || tee == nullptr || record_queue == nullptr ||
        split_sink == nullptr || fake_queue == nullptr || fake_sink == nullptr) {
        std::cerr << "Failed to create one or more GStreamer elements" << std::endl;

        if (pipeline != nullptr) {
            gst_object_unref(pipeline);
        }

        return 1;
    }

    // 6. Element 속성 설정
    g_object_set(
        source,
        "location", rtsp_uri.c_str(),
        "latency", 100,
        nullptr
    );

    g_object_set(
        parser,
        "config-interval", -1,
        nullptr
    );

    g_object_set(
        split_sink,
        "location", output_pattern.c_str(),
        "max-size-time", static_cast<guint64>(5 * GST_SECOND),
        nullptr
    );

    g_object_set(
        fake_sink,
        "sync", FALSE,
        nullptr
    );

    // 7. 파이프라인에 Element 추가
    gst_bin_add_many(
        GST_BIN(pipeline),
        source,
        depay,
        parser,
        tee,
        record_queue,
        split_sink,
        fake_queue,
        fake_sink,
        nullptr
    );

    // 8. 정적 Pad 구간 연결
    if (!gst_element_link_many(depay, parser, tee, nullptr)) {
        std::cerr << "Failed to link depay, parser, tee" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }

    if (!gst_element_link(record_queue, split_sink)) {
        std::cerr << "Failed to link record_queue and split_sink" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }

    if (!gst_element_link(fake_queue, fake_sink)) {
        std::cerr << "Failed to link fake_queue and fake_sink" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }

    // 9. tee의 request pad 생성
    GstPad* tee_record_pad = gst_element_request_pad_simple(tee, "src_%u");
    GstPad* tee_fake_pad = gst_element_request_pad_simple(tee, "src_%u");

    GstPad* record_queue_sink_pad = gst_element_get_static_pad(record_queue, "sink");
    GstPad* fake_queue_sink_pad = gst_element_get_static_pad(fake_queue, "sink");

    if (tee_record_pad == nullptr || tee_fake_pad == nullptr ||
        record_queue_sink_pad == nullptr || fake_queue_sink_pad == nullptr) {
        std::cerr << "Failed to get tee or queue pads" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }

    // 10. tee branch 연결
    if (gst_pad_link(tee_record_pad, record_queue_sink_pad) != GST_PAD_LINK_OK) {
        std::cerr << "Failed to link tee to record queue" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }

    if (gst_pad_link(tee_fake_pad, fake_queue_sink_pad) != GST_PAD_LINK_OK) {
        std::cerr << "Failed to link tee to fake queue" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }

    // 11. Dynamic Pad 콜백 등록
    AppData data;
    data.depay = depay;

    g_signal_connect(
        source,
        "pad-added",
        G_CALLBACK(on_pad_added),
        &data
    );

    // 12. 파이프라인 실행
    GstStateChangeReturn ret = gst_element_set_state(pipeline, GST_STATE_PLAYING);

    if (ret == GST_STATE_CHANGE_FAILURE) {
        std::cerr << "Failed to set pipeline to PLAYING" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }

    // 13. 15초 동안 녹화
    std::cout << "Recording for 15 seconds..." << std::endl;
    g_usleep(15 * G_USEC_PER_SEC);

    // 14. 파일 마무리를 위해 EOS 전송
    std::cout << "Sending EOS..." << std::endl;
    gst_element_send_event(pipeline, gst_event_new_eos());

    // 15. Bus 가져오기
    GstBus* bus = gst_element_get_bus(pipeline);

    // 16. ERROR 또는 EOS 메시지 대기
    GstMessage* msg = gst_bus_timed_pop_filtered(
        bus,
        GST_CLOCK_TIME_NONE,
        static_cast<GstMessageType>(GST_MESSAGE_ERROR | GST_MESSAGE_EOS)
    );

    // 17. 메시지 처리
    if (msg != nullptr) {
        switch (GST_MESSAGE_TYPE(msg)) {
        case GST_MESSAGE_ERROR: {
            GError* err = nullptr;
            gchar* debug_info = nullptr;

            gst_message_parse_error(msg, &err, &debug_info);

            std::cerr << "GStreamer ERROR: "
                      << (err ? err->message : "unknown error")
                      << std::endl;

            std::cerr << "Debug info: "
                      << (debug_info ? debug_info : "none")
                      << std::endl;

            if (err != nullptr) {
                g_error_free(err);
            }

            if (debug_info != nullptr) {
                g_free(debug_info);
            }

            break;
        }

        case GST_MESSAGE_EOS:
            std::cout << "End of stream" << std::endl;
            break;

        default:
            std::cout << "Unexpected message" << std::endl;
            break;
        }

        gst_message_unref(msg);
    }

    // 18. Bus 해제
    gst_object_unref(bus);

    // 19. 파이프라인 정지
    gst_element_set_state(pipeline, GST_STATE_NULL);

    // 20. tee request pad 해제
    gst_element_release_request_pad(tee, tee_record_pad);
    gst_element_release_request_pad(tee, tee_fake_pad);

    gst_object_unref(tee_record_pad);
    gst_object_unref(tee_fake_pad);
    gst_object_unref(record_queue_sink_pad);
    gst_object_unref(fake_queue_sink_pad);

    // 21. 파이프라인 해제
    gst_object_unref(pipeline);

    std::cout << "Recording finished" << std::endl;

    return 0;
}