...
bool CameraPipeline::create() {
    ...
    // 10. splitmuxsink 파일명 생성 콜백 등록
    g_signal_connect(
        split_sink_,
        "format-location",
        G_CALLBACK(&CameraPipeline::on_format_location),
        this
    );
...
void CameraPipeline::stop() {
    ...
    // 2. EOS로 마지막 파일 마무리 시도
    bool finalized = finalize_recording(3000);

    // 3. EOS로 마무리하지 못한 recording segmentr는 interrupted 처리
    if (!finalized && segment_store_ != nullptr) {
        segment_store_->mark_open_segments_interrupted(camera_id_);
    }

    // 4. 파이프라인 정지
    gst_element_set_state(pipeline_, GST_STATE_NULL);

// 5. tee request pad 해제
...
void CameraPipeline::handle_pad_added(GstPad* new_pad) {
    ...
    std::cout << "[" << camera_id_ << "] Detected RTP video encoding-name: "
              << (encoding_name ? encoding_name : "none")
              << std::endl;

    // 6. encoding-name을 VideoStreamProfile로 변환
    //    26강의 핵심은 CameraPipeline 안에 코덱 매핑표를 직접 두지 않는 것이다.
    VideoStreamProfile profile = make_video_stream_profile_from_encoding_name(
        encoding_name ? encoding_name : ""
    );

    std::cout << "[" << camera_id_ << "] Selected video stream profile: "
              << profile.name
              << " codec=" << video_codec_to_string(profile.codec)
              << " depay=" << (profile.depay_element.empty() ? "none" : profile.depay_element)
              << " parser=" << (profile.parser_element.empty() ? "none" : profile.parser_element)
              << " decoder=" << (profile.decoder_element.empty() ? "none" : profile.decoder_element)
              << " supported=" << (profile.supported ? "true" : "false")
              << std::endl;

    // 7. 감지는 했지만 현재 VMS 정책상 미지원인 포맷은 여기서 중단한다.
    if (!is_supported_video_profile(profile)) {
        std::cout << "[" << camera_id_ << "] Unsupported RTP video encoding-name: "
                  << (encoding_name ? encoding_name : "none")
                  << std::endl;
    ...
    // 8. 26강 시점의 실제 파이프라인은 아직 H264 고정이다.
    //    H265 profile은 만들었지만, rtph265depay/h265parse/avdec_h265를
    //    동적으로 생성하는 작업은 28~31강에서 진행한다.
    if (profile.codec != VideoCodec::H264) {
        std::cout << "[" << camera_id_ << "] Profile is recognized, but this lesson still links only H264. "
                  << "Dynamic downstream creation will be added later. codec="
                  << video_codec_to_string(profile.codec)
                  << std::endl;
    ...
    // 9. rtph264depay의 sink pad를 가져온다.
    ...
    // 10. 이미 연결되어 있으면 같은 pad를 중복 연결하지 않는다.
    ...
    // 11. rtspsrc의 동적 pad를 rtph264depay에 연결한다.
...
gchar* CameraPipeline::handle_format_location(guint fragment_id) {
    // 1. 파일명 생성
    gchar* path = g_strdup_printf(
        record_pattern_.c_str(),
        fragment_id
    );
    ...
    if (segment_store_ != nullptr) {
        segment_store_->on_segment_started(
            camera_id_,
            path
        );
    }

    // 3. splitmuxsink가 나중에 g_free()  할 수 있도록 반환
    return path;
}

bool CameraPipeline::finalize_recording(int timeout_ms) {
    // 1. 파이프라인 존재 여부 확인
    ...
    // 3. EOS 전송
    std::cout << "[" << camera_id_ << "] Sending EOS to finalize recording..."
              << std::endl;

    gboolean sent = gst_element_send_event(
        pipeline_,
        gst_event_new_eos()
    );

    if (!sent) {
        std::cerr << "[" << camera_id_ << "] Failed to send EOS" << std::endl;
        gst_object_unref(bus);
        return false;
    }

    // 4. timeout까지 bus 메시지 처리
    gint64 start_time = g_get_monotonic_time();
    gint64 timeout_us = static_cast<gint64>(timeout_ms) * 1000;
    ...
    while (true) {
        ...
        gint64 remain_us = timeout_us - elapsed;

        GstMessage* msg = gst_bus_timed_pop_filtered(
            bus,
            static_cast<GstClockType>(remain_us) * GST_USECOND,
            static_cast<GstMessageType>(
                GST_MESSAGE_ERROR |
                GST_MESSAGE_EOS |
                GST_MESSAGE_ELEMENT
    ...
    if (eos_received) {
        std::cout << "[" << camera_id_ << "] Recordding finalize by EOS"
                  << std::endl;
    } else {
        std::cout << "[" << camera_id_ << "] EOS finalize timeout or failed"
                  << std::endl;
    }
...
void CameraPipeline::handle_bus_message(GstMessage* msg) {
...
void CameraPipeline::handle_element_message(GstMessage* msg) {
    // 1. 메시지 구조 확인
    ...
    // 2. splitmuxsink fragment closed 메시지만 처리
    if (g_strcmp0(name, "splitmuxsink-fragment-closed") != 0) {
        return;
    }

    // 3. location 필드 확인
    const gchar* location = gst_structure_get_string(
        structure,
        "location"
    );
    ...
    if (location == nullptr) {
        std::cout << "[SEGMENT FINISHED] "
                  ...
                  << ", warning=location field not found"
                  << std::endl;
        return;
    }
    ...
    // 4. Segment 종료 기록
    if (segment_store_ != nullptr) {
        segment_store_->on_segment_finished(
            camera_id_,
            location
        );
    }
...
gchar* CameraPipeline::on_format_location(
    GstElement* splitmux,
    guint fragment_id,
    gpointer user_data
) {
    ...
    return self->handle_format_location(fragment_id);
}