...
int main(int argc, char* argv[]) {
    ...
    // 3. 파이프라인 생성
    GstElement* pipeline = gst_pipeline_new("rtsp-split-record-pipeline");
    ...
    if (pipeline == nullptr || source == nullptr || depay == nullptr ||
        parser == nullptr || split_sink == nullptr) {
    ...
    // 8. 정적 Pad 구간 연결
    if (!gst_element_link_many(depay, parser, split_sink, nullptr)) {
        std::cerr << "Failed to link depay, parser, splitmuxsink" << std::endl;
        gst_object_unref(pipeline);
        return 1;
    }
