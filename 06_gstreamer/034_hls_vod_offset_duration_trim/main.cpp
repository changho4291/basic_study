...
int main(int argc, char* argv[]) {
    ...
    std::cout << std::endl;
    std::cout << "===== Lesson 37 HLS VOD Offset/Duration Trim Test =====" << std::endl;
    std::cout << "Create playback sessions for the last 10 minutes, create HLS VOD session directories, write index.m3u8, then convert every MP4 segment plan to TS files with offset/duration trim." << std::endl;
    std::cout << "This step applies source_offset_ms and duration_ms to the GStreamer conversion seek range." << std::endl;
    ...
    for (const auto& config : camera_configs) {
        ...
        //     index.m3u8를 작성한다.
        //     37강에서는 이 playlist가 가리키는 TS 파일들을 요청 구간 기준으로 생성한다.
        if (hls_result.ok && hls_result.session.has_value()) {
            ...
            // 15. 여러 MP4 segment -> TS 변환
            //     이번 단계에서는 HLS session의 모든 segment plan을 변환한다.
            //     source_offset_ms와 duration_ms를 GStreamer seek 구간으로 반영한다.
            HlsVodConverter converter;
            auto convert_result = converter.convert_all_segments(*hls_result.session);
