...
int main(int argc, char* argv[]) {
    ...
    std::cout << std::endl;
    std::cout << "===== Lesson 35 Single MP4 to HLS TS Conversion Test =====" << std::endl;
    std::cout << "Create playback sessions for the last 10 minutes, create HLS VOD session directories, write index.m3u8, then convert the first MP4 segment to seg_000.ts." << std::endl;
    std::cout << "This step converts only the first HLS segment plan. Offset/duration trimming is not applied yet." << std::endl;
    ...
    for (const auto& config : camera_configs) {
        ...
        //     index.m3u8를 작성한다.
        //     35강에서는 이 playlist가 가리키는 첫 번째 TS 파일도 생성한다.
        if (hls_result.ok && hls_result.session.has_value()) {
            ...
            // 15. 단일 MP4 segment -> TS 변환
            //     이번 단계는 PoC다. 첫 번째 segment plan만 변환한다.
            //     source_offset_ms와 duration_ms는 아직 실제 trim에 반영하지 않는다.
            HlsVodConverter converter;
            auto convert_result = converter.convert_first_segment(*hls_result.session);
            HlsVodConverter::print_convert_result(convert_result);
        }

        // 16. Playback seek 테스트
