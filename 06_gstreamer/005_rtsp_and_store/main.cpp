...
int main(int argc, char* argv[]) {
    ...
    std::string output_file = "record_cpp.mp4";

    if (argc >= 2) { rtsp_uri = argv[1]; }
    if (argc >= 3) { output_file = argv[2]; }

    // 3. 파이프라인 생성
    GstElement* pipeline = gst_pipeline_new("rtsp-record-pipeline");
    ...
    if (pipeline == nullptr || source == nullptr || depay == nullptr ||
        parser == nullptr || muxer == nullptr || sink == nullptr) {
    ...
    if (!gst_element_link_many(depay, parser, muxer, sink, nullptr)) {
        std::cerr << "Failed to link depay, parser, muxer, sink" << std::endl;
    ...
    std::cout << "Recording finished: " << output_file << std::endl;
