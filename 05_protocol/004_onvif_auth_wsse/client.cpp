...
constexpr const char* kDefaultEndpoint = "http://127.0.0.1:8082/onvif/device_service";
...
void print_usage(const char* program) {
    std::printf("Usage: %s [--endpoint url] [--username name] [--password value] [--timeout seconds] [--no-auth]\n",
                program);
}
...
int main(int argc, char** argv) {
    ...
    soap_set_sent_logfile(soap, "auth_sent.xml");
    soap_set_recv_logfile(soap, "auth_recv.xml");

    if (!options.no_auth) {
        if (!wsse_auth::add_username_token_digest(
                soap,
                options.username,
                options.password)) {
            std::fprintf(stderr, "failed to add UsernameToken Digest\n");
            soap_destroy(soap);
            soap_end(soap);
            soap_free(soap);
            return 1;
    ...
    std::printf("Send authenticated ONVIF GetDeviceInformation\n");
    std::printf("Endpoint: %s\n", options.endpoint);
    std::printf("Username: %s\n", options.no_auth ? "(none)" : options.username);
    std::printf("Password mode: %s\n", options.no_auth ? "none" : "UsernameToken Digest");
    ...
    if (soap_call_tds__GetDeviceInformation(
        ...
        std::printf("\nSent log: auth_sent.xml\n");
        std::printf("Recv log: auth_recv.xml\n");
        soap_destroy(soap);
        soap_end(soap);
        soap_free(soap);
        return 1;
